  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-md">
      <div className="relative group">
        ...
        <input
          type="text"
          className="block w-full pl-10 pr-10 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all shadow-sm"
          ...
          >
            <div className="max-h-[400px] overflow-y-auto p-2 space-y-4">
              ...
              {results.products.length > 0 && (
                <div>
                  <h4 className="px-3 py-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    ...
                    {results.products.map((p) => (
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400">Stock: {p.stock_quantity}</p>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-sm font-black text-indigo-600">{formatCurrency(p.selling_price)}</span>
