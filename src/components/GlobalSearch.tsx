...
import { Search, Loader2, Package, User, ChevronRight, X, Building2 } from 'lucide-react';
...
export default function GlobalSearch() {
  ...
  const [results, setResults] = useState<{ products: any[], customers: any[], suppliers: any[] }>({ products: [], customers: [], suppliers: [] });
  ...
  const handleSelect = (type: 'product' | 'customer' | 'supplier', id: string | number) => {
    setShowResults(false);
    setQuery('');
    if (type === 'product') {
  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-md">
      <div className="relative group">
        ...
        {query && (
          <button
            onClick={() => { setQuery(''); setResults({ products: [], customers: [], suppliers: [] }); }}
        ...
        {showResults && (
          ...
          <div className="max-h-[300px] overflow-y-auto p-1.5 space-y-3">
            ...
            {results.suppliers && results.suppliers.length > 0 && (
                  ...
                  {results.suppliers.map((s) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <span className={cn(
                          "text-[13px] font-black",
                          s.outstanding_balance > 0 ? "text-rose-600" : "text-emerald-600"
                        )}>
              ...
              {results.products.length === 0 && results.customers.length === 0 && (!results.suppliers || results.suppliers.length === 0) && (
